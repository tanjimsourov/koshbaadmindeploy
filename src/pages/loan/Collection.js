...
const Collection = () => {
    ...
    const  loan = Number(LoanID);
    const installmentAmount = Number(ia)
    ...
    const data = {loan,installmentAmount}

    const [message, setMessage] = useState('')
    const [error, setError] = useState('')

    async function submitHandler() {
        await axios.post(`${baseurl}/api/addcollection`, data, config)
            .then(resp => {
                if (resp.status === 201) {
        ...
        <div>
            ...
            <div style={{padding: "5px 5px 20px 580px", backgroundColor: "#f5f5f5"}}>
                <div className="newLoanee">
                    <h1 className="addLoaneeTitle">Add Collection</h1>
